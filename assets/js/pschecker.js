...
jQuery.fn.password_strength = function(options)
{
	...
	{
		var container = null, jQuerybar = null;
		if (settings.container)
		{
			container = jQuery(settings.container);
		}
		else
		{
			container = jQuery('<span/>').attr('class', 'password_strength');
			jQuery(this).after(container);
		}

		if (settings.bar)
		{
			jQuerybar = jQuery(settings.bar);
		}
		
		jQuery(this).bind('keyup.password_strength', function()
		{
			...
			if (val.length > 0)
			{
				...
				if (jQuerybar && !jQuerybar.hasClass(_barClass))
				{
					jQuerybar.attr('class', 'password_bar ' + _barClass);
				}
			}
			else
			{
				...
				if (jQuerybar) {
					jQuerybar.attr('class', 'password_bar');
				}
			}
			if (settings.onCheck) {
		...
		if (jQuery(this).val() != '') { // thanks Jason Judge
				jQuery(this).trigger('keyup.password_strength');
		}
	});
};
