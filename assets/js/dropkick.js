...
(function ($, window, document) {
  ...
  var
    ...
    dropdownTemplate = [
      '<div class="dk_container {{ isDisabled }}" id="dk_container_{{ id }}" select-field-name="{{ selectFieldName }}" tabindex="{{ tabindex }}"><div class="controller-select"></div>',
        '<a class="dk_toggle">',
  ...
  methods.init = function (settings) {
    ...
    return this.each(function () {
      ...
      setTimeout(function () {
        $select.css({'visibility':'hidden','position':'absolute','left':'-9999px'});
      }, 0);
    });
  };
  ...
  function _validatingFeilds(option) {
    var
      value = option.attr('data-dk-dropdown-value'),
      parentControl = option.parents('.required-select-field');

   if(value) {
      if(parentControl.hasClass('error')) {
  ...
  function _build (tpl, view) {
    ...
    template = template.replace('{{ selectFieldName }}', view.id);
  ...
  $(function () {
    ...
    $('.dk_toggle, .controller-select').live('click', function (e) {
    ...
    $('.dk_options a').live(($.browser.msie ? 'mousedown' : 'click'), function (e) {
      ...
      _validatingFeilds($option); // Added validation
      e.preventDefault();
      return false;
    });
